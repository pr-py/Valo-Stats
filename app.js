...
const userAction = async () => {
    ...
    if (pname.split('#').length != 2) {
        ...
        rank = myJson1['data'][0]['currenttierpatched'];
        rank_img = myJson1['data'][0]['images']['small'];
        ...
        for (y = 0; y < 5; y++) {
            ...
            for (j = 0; j < 10; j++) {
                ...
                if (myJson['data'][y]['players']['all_players'][j]['name'].normalize('NFC') === pname.normalize('NFC') && myJson['data'][y]['players']['all_players'][j]['tag'].normalize('NFC') === ptag.normalize('NFC') && mode.toLowerCase() == mode_list[md]) {
                    ...
                    level = myJson['data'][y]['players']['all_players'][j]['level'];
            ...
            for (i in dd) {
                var assists, score, kd, kills, deaths;
                if (dd[i]['name'] == pname) {
